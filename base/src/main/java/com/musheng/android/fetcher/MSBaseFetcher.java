    ...
    public Disposable enqueue(final R request,  MSFetcherResponse<R, E> response, final MSFetcherIndicator indicator){
        ...
        return Observable.create(new ObservableOnSubscribe<E>() {
            ...
            public void subscribe(ObservableEmitter<E> emitter) throws Exception {
                Collection<MSFetcherEntityProvider<R, E>> entityProviders = providerMap.values();
                if(entityProviders.isEmpty()) {
                    emitter.onError(new MSFetcherThrowable( "Unknown Error"));
                    return;
                }
                ...
                for(MSFetcherEntityProvider<R, E> provider : entityProviders){
                    ...
                    E temp = provider.getEntity(request);
                    if(temp != null){
                        handleIndex = index;
                        entity = temp;
                ...
                if (entity == null) {
                    emitter.onError(new MSFetcherThrowable("Fail"));
                    return;
                }
                ...
                if(!isCancel && (indicator == null || !indicator.isCancel())){
                    if(throwable instanceof MSFetcherThrowable){
                        ...
                        if(message.toLowerCase().startsWith("failed to connect to")){
                            finalResponse.onError(new MSFetcherThrowable("Network error"));
                        } else {
                            finalResponse.onError(new MSFetcherThrowable(message));
                        }
                    }
                    for(MSFetcherResponse<R,E> resp : responseList){
                        if(throwable instanceof MSFetcherThrowable){
